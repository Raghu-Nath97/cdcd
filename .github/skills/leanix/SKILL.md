...
4. Navigates to the Diagrams module (`/<workspace>/diagrams/overview/all-diagrams`), searches for the app name, collects links to matching diagram pages.
  ...
  "diagrams": [
    {
      ...
      "diagram_url": "https://pg.leanix.net/PGPROD/diagrams/<uuid>",
...
| Workspace landing | `https://pg.leanix.net/PGPROD/` |
| Diagrams module (all diagrams) | `https://pg.leanix.net/PGPROD/diagrams/overview/all-diagrams` |
| Inventory (search Fact Sheets) | `https://pg.leanix.net/PGPROD/inventory` |
...
**Workspace slug is `PGPROD` (case-sensitive).** Earlier builds of this skill assumed `pg`; that was wrong. If a non-prod workspace ever needs to be added (e.g. `PGTEST`), pass it via `--workspace`.
...
- The specific Diagrams-module URL path (`/<workspace>/diagrams/overview/all-diagrams`) — correct for current LeanIX builds, but could change. If the script reports it could not reach the Diagrams page, verify the URL in a real browser before blaming the script.
...
Be honest. Say: "the current LeanIX fetch does not cover that — the script is scoped to diagrams-per-app. To answer this, we would need to extend the script per Section 10, or you can open LeanIX directly at `https://pg.leanix.net/PGPROD/`".
